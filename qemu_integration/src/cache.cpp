...
Cache::Cache(uint32_t s, uint32_t a, uint32_t l) : num_sets(s), assoc(a), line_size(l) {
...
void Cache::read(uint64_t addr, uint64_t* data, CacheState *mesi_state, bool* hit) {
...
void Cache::write(uint64_t addr, CacheState mesi_state, uint64_t* data, uint64_t write_size, uint64_t* wb_data, uint64_t *wb_addr, bool* wb_valid) {
    uint64_t set_no = (addr >> intLog2(line_size)) % num_sets;
    uint64_t tag    = (addr >> (intLog2(line_size) + intLog2(num_sets)));
    ...
    uint64_t evict_idx;
    #ifdef DEBUG
        std::cout << "Set: " << set_no << ", Tag: " << tag << ", Block Index: " << block_idx << std::endl;
    #endif
    // Search for the line in the set
    for (uint32_t i = 0; i < assoc; ++i) {
        if ((sets[set_no].lines[i].mesi_state == MESI_EXCLUSIVE || sets[set_no].lines[i].mesi_state == MESI_MODIFIED) && sets[set_no].lines[i].tag == tag) {
            ...
            sets[set_no].lines[i].mesi_state = mesi_state;
            return;
        } else {
    ...
    printf("Cache Miss: Evicting line from set %lu, way %lu\n", set_no, sets[set_no].fifo_ptr);
    ...
    if (miss && (sets[set_no].lines[evict_idx].mesi_state == MESI_MODIFIED)) {
        ...
        *wb_valid = true;
        ...
        sets[set_no].lines[evict_idx].mesi_state = mesi_state;
